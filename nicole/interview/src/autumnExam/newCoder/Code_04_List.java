...
public class Code_04_List {
    ...
    public static class IsPalindromeList {       //判断该链表是否为回文结构
        public boolean isPalindrome1(Node head) {    //方法1:空间O(n)，全部入栈
        ...
        public boolean isPalindrome2(Node head) {       //方法2:空间O(n/2)，使用快慢指针，只将后半段入栈
            if (head == null || head.next == null) {
                return true;
            }
            Node fast = head;
            Node slow = head.next;      //慢指针先走一步 原因：如果链表为奇数，则当后半段入栈时，可以省去判断最中间的节点值。
         ...
         public boolean  isPalindrome3(Node head){    //方法三：走到中间将链表逆序，空间复杂度O(1)
             Node fast = head;
             Node slow = head;    //这里不可以快走1步
             while (fast.next!= null && fast.next.next != null) {
                 slow = slow.next;
                 fast = fast.next.next;
             }

             Node p =  slow.next;     //将链表逆序，复杂，从slow节点（中间节点）开始逆序
             fast = p.next;
             p.next = slow;
             slow.next = null;
             slow = p;
             ...
             p = fast.next;     //最后将链表还原，fast为链表结尾的空指针。
             fast.next = null;
             while(p != null){
                 slow = p.next;
                 p.next = fast;
